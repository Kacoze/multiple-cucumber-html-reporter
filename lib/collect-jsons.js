    ...
    if (files.length > 0) {
        files.map(file => {
            ...
            JSON.parse(data).map(json => {
                ...
                const filePath = file.split('\\')[file.split('\\').length - 1];
                const fileData = filePath.replace(/^.*[\\\/]/, '').split('.');
                json.metadata.brand = fileData[0];
                json.metadata.app = fileData[1];
                json.metadata.branch = fileData[2];
                json.metadata.runType = fileData[3];
